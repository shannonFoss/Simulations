...
public class Warehouse extends Module
{
	...
	public Warehouse()
	{
		...
		{
			fnum = i+1;
			leftFloor[i] = new WarehouseFloor("LeftFloor " + fnum,WarehouseFloor.MIDDLE,fnum,eLeft);
			rightFloor[i] = new WarehouseFloor("RightFloor " + fnum,WarehouseFloor.MIDDLE,fnum, eRight);
			leftFloor[i-1].connectUp(leftFloor[i]);
			rightFloor[i-1].connectUp(rightFloor[i]);
			addSubmodule(leftFloor[i]);
			addSubmodule(rightFloor[i]);
		}
		
		fnum++;
		leftFloor[NUMBER_OF_FLOORS-1] = new WarehouseFloor("LeftFloor " + fnum,WarehouseFloor.TOP,fnum,eLeft);
		rightFloor[NUMBER_OF_FLOORS-1] = new WarehouseFloor("RightFloor " + fnum,WarehouseFloor.TOP,fnum,eRight);
		addSubmodule(leftFloor[i]);
		addSubmodule(rightFloor[i]);
